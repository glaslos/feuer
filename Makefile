...
prometheus:
	$(DOCKER) -p 9090:9090 -v $(HOME)/prometheus.yml:/etc/prometheus/prometheus.yml prom/prometheus:latest

influxdb:
	$(DOCKER) -p 8086:8086 quay.io/influxdb/influxdb:v2.0.3

telegraf:
	$(DOCKER) -v $(HOME)/telegraf.conf:/etc/telegraf/telegraf.conf telegraf:latest
